...
def cadastrar_projeto():
    ...
    valor = request.form['valor']
    objetivo = request.form['objetivo']
    linha_credito = request.form['linha_credito']
    produtor = request.form['produtor']

    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", size=12)
    pdf.cell(200, 10, txt="Proposta de Projeto - Sistema Topogeo", ln=True, align='C')
    pdf.ln(10)
    pdf.cell(200, 10, txt=f"Nome do Projeto: {nome_projeto}", ln=True)
    pdf.cell(200, 10, txt=f"Valor: R$ {valor}", ln=True)
    pdf.cell(200, 10, txt=f"Objetivo: {objetivo}", ln=True)
    pdf.cell(200, 10, txt=f"Linha de Crédito: {linha_credito}", ln=True)
    pdf.cell(200, 10, txt=f"Produtor: {produtor}", ln=True)

    pdf_file = "/tmp/proposta.pdf"
    pdf.output(pdf_file)
